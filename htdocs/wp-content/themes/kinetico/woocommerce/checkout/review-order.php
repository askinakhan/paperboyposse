...
<div id="order_review">
	...
	<div class="box-striped review-box">

	<h3 class="title" id="order_review_heading"><?php _e('Your order', 'kinetico'); ?></h3>

	<table class="shop_table">
		<thead>
			<tr>
				<th class="product-name"><?php _e('Product', 'kinetico'); ?></th>
				<th class="product-quantity"><?php _e('Qty', 'kinetico'); ?></th>
				<th class="product-total"><?php _e('Totals', 'kinetico'); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php
	...
	<div class="box-striped tax-details">
		<table>
		<tbody>
		
			<tr class="cart-subtotal">
				<th colspan="2"><?php _e('Cart Subtotal', 'kinetico'); ?></th>
			...
			<tr class="discount">
				<th colspan="2"><?php _e('Cart Discount', 'kinetico'); ?></th>
			...
			<tr class="shipping">
				<th colspan="2"><?php _e('Shipping', 'kinetico'); ?></th>
				<td>
				<?php
					...
					// No shipping methods are available
						...
						if ( ! $woocommerce->customer->get_shipping_country() || ! $woocommerce->customer->get_shipping_state() || ! $woocommerce->customer->get_shipping_postcode() ) {
							echo '<p>'.__('Please fill in your details above to see available shipping methods.', 'kinetico').'</p>';
						} else {
					...
					if (sizeof($taxes)>0) :
						...
						foreach ($taxes as $key => $tax) : if ($woocommerce->cart->tax->is_compound( $key )) : $has_compound_tax = true; continue; endif;
							...
							<tr class="tax-rate tax-rate-<?php echo $key; ?>">
								<th colspan="2"><?php if (get_option('woocommerce_prices_include_tax')=='yes') : _e('incl.', 'kinetico'); endif; ?> <?php echo $woocommerce->cart->tax->get_rate_label( $key ); ?></th>
						...
						if ($has_compound_tax && !$woocommerce->cart->prices_include_tax) :
							...
							<tr class="order-subtotal">
								<th colspan="2"><?php _e('Order Subtotal', 'kinetico'); ?></th>
			...
			<tr class="discount">
				<th colspan="2"><?php _e('Order Discount', 'kinetico'); ?></th>
			...
			<tr class="total">
				<th colspan="2"><?php _e('Order Total', 'kinetico'); ?></th>
	...
	<div id="payment">
		...
		<div class="box-striped payment-methods">
		
		<h3 class="title"><?php _e('Payment Method', 'kinetico'); ?></h3>
			...
			<?php 
				...
				if ($available_gateways) : 
					...
					if ( !$woocommerce->customer->get_country() ) :
						echo '<p>'.__('Please fill in your details above to see available payment methods.', 'kinetico').'</p>';
					else :
						echo '<p>'.__('Sorry, it seems that there are no available payment methods for your state. Please contact us if you require assistance or wish to make alternate arrangements.', 'kinetico').'</p>';
					endif;
		...
		<div class="form-row place-order">
			...
			<noscript><?php _e('Since your browser does not support JavaScript, or it is disabled, please ensure you click the <em>Update Totals</em> button before placing your order. You may be charged more than the amount stated above if you fail to do so.', 'kinetico'); ?><br/><input type="submit" class="button-alt" name="woocommerce_checkout_update_totals" value="<?php _e('Update totals', 'kinetico'); ?>" /></noscript>
		
			<?php $woocommerce->nonce_field('process_checkout')?>
			
			<?php do_action( 'woocommerce_review_order_before_submit' ); ?>
			
			<input type="submit" class="button alt" name="woocommerce_checkout_place_order" id="place_order" value="<?php echo apply_filters('woocommerce_order_button_text', __('Place Order', 'kinetico')); ?>" />
			
			<?php if (woocommerce_get_page_id('terms')>0) : ?>
			<p class="form-row terms">
				<label for="terms" class="checkbox"><?php _e('I accept the', 'kinetico'); ?> <a href="<?php echo esc_url( get_permalink(woocommerce_get_page_id('terms')) ); ?>" target="_blank"><?php _e('terms &amp; conditions', 'kinetico'); ?></a></label>
