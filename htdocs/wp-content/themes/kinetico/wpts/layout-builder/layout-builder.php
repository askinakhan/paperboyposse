...
/*** PORTFOLIO META OPTIONS ***/
	...
	{  
			...
			if (@base64_decode( $elements, true )) {
				...
				if($custom["import-wpts"][0] != '' && $custom["import-wpts"][0] != null) {
					$imp = $custom["import-wpts"][0];
					...
					$imp = maybe_unserialize($imp);
					$elements = $imp;
					update_post_meta($post->ID, "import-wpts", null);
				}   
    ?>  
		...
		<textarea name="import-wpts" class="import"></textarea>
		</div>
    ...
    function layout_builder_save(){  
        ...
        if ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE ){  
			...
			if($_POST["import-wpts"] != "")
			{	
				update_post_meta($post->ID, "import-wpts", $_POST["import-wpts"]);
			}
