...
class wptsGenerator {
	...
	function save_skin_style() {
	        ...
	        $fhandle = fopen(ABSPATH.'wp-content/themes/kinetico/css/base.css', 'w+');
			$content =  $this->get_include_contents(ABSPATH.'wp-content/themes/kinetico/css/base.php');
					...
					foreach($ops as $slide) {
						...
						<input type="hidden" name="sliders[<?php echo $id; ?>]" />
							...
							<select name="sliders[<?php echo $id; ?>][1]">
								<option value="0" <?php if($slide[1] == "0") echo 'selected="selected"';?>>Home Slider</option>
							</select>
							<?php 
								$name = $slide[2];
								if($name == "")
									$name = "Slider Name";
							?>
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="sliders[<?php echo $id; ?>][2]" value="<?php echo $name; ?>" onfocus="if(this.value == 'Slider Name') this.value = '';" onblur="if(this.value == '') this.value = 'Slider Name';" />
						...
						<div class="slides">
							<?php 
								if($slide[1] == 0)
									normal_slider($slide[3], $id, $idinner); 
							?>
						</div> <!-- slides -->
	...
	function sidebar($value) {
		wp_enqueue_style( 'wpts-sidebarmanager', get_template_directory_uri() . '/wpts/admin/sidebar_manager/sidebar_manager.css', array(), NULL, 'all' );
		wp_enqueue_script( 'wpts-sidebarmanager', get_template_directory_uri() . '/wpts/admin/sidebar_manager/sidebar_manager.js', array('jquery'), NULL );
	?>
	<div class="sidebars-custom">
		<?php 
			if (isset($this->saved_options[$value['id']])) {	
				...
				foreach ($sides as $side) {
					if($side != '') {
						echo '<div class="sidebar"><input type="text" value="'.$side.'" /><a href="#">Remove</a></div>';
					}
				}
			}
		?>
	...
	<div class="wpts_input wpts_text">
	<label for="new-sidebar">New Sidebar</label>
 	<input type="text" id="new-sidebar" name="new-sidebar" value="" /><a id="new-sidebar-btn" href="#">Add New Sidebar</a>
	</div>

	<input name="<?php echo $value['id']; ?>" id="<?php echo $value['id']; ?>" class="final-sidebars" type="hidden" value="<?php if (isset($this->saved_options[$value['id']])) {	echo stripslashes($this->saved_options[$value['id']]);	} else { echo $value['default'];} ?>" />
