	...
	*/
	...
	function sf_woo_links($position, $config = "") {
		...
		if (is_user_logged_in()) {
			global $current_user;
			get_currentuserinfo();
			$woo_links_output .= '<li class="tb-welcome">' . __("Welcome") . " " . $current_user->display_name . '</li>'. "\n";
		} else {
			$woo_links_output .= '<li class="tb-welcome">' . __("Welcome") . '</li>'. "\n";
		}
		if ($ss_enable) {
	...
	function sf_get_cart() {
		...
		if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            ...
            if ( sizeof($woocommerce->cart->cart_contents)>0 ) {
            	...
            	$cart_output .= '<div class="bag-header">'.sprintf(_n('%d item in the shopping bag', '%d items in the shopping bag', $woocommerce->cart->cart_contents_count, 'woothemes'), $woocommerce->cart->cart_contents_count).'</div>';
