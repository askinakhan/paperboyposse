...
class SwiftPageBuilderShortcode_spb_gmaps extends SwiftPageBuilderShortcode {
    ...
    protected function content( $atts, $content = null ) {
        ...
        if ($map_coords == "error") {
        $output .= '<div class="spb_map_error">Please enable URL File Access on your hosting.</div>';
        } else {
        $output .= '<div class="spb_map_wrapper"><div class="map-canvas" style="width:100%;height:'.$size.'px;" data-address="'.$address.'" data-lat="'.$map_coords['lat'].'" data-long="'.$map_coords['long'].'" data-zoom="'.$zoom.'" data-maptype="'.$type.'" data-pinimage="'.$img_url[0].'"></div></div>';
        }
        $output .= "\n\t\t".'</div> '.$this->endBlockComment('.spb_wrapper');
        $output .= "\n\t".'</div> '.$this->endBlockComment($width);
		if ($fullscreen != "yes") {
